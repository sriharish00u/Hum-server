...
export function setupSocketHandlers(io: Server): void {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("send-discussion-message", async (data: { threadId: string; userId: string; userName: string; text: string }) => {
      try {
        const { threadId, userId, userName, text } = data;
        if (userId !== currentUserId) {
          socket.emit("error", { message: "Cannot send message as another user" });
          return;
        }
        ...
        socket.to(threadId).emit("receive-discussion-message", { threadId, message });
      } catch (error) {
        logger.error({ err: error }, 'error in send-discussion-message handler');
      }
    });
  });
}